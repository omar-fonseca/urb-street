...
// ÔöÇÔöÇÔöÇ DATA ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const HATS = [
...
// ÔöÇÔöÇÔöÇ WA HELPER ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function waLink(wa: string, prenda?: string) {
  const msg = prenda
    ? `Hola, me interesa esta prenda: *${prenda}*. ┬┐Est├í disponible y cu├íl es el precio?`
    : "Hola URB&STREET, quiero hacer un pedido.";
  return `https://wa.me/${wa}?text=${encodeURIComponent(msg)}`;
}

// ÔöÇÔöÇÔöÇ SHARED ICONS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
          ...
          {(category.comingSoonItems ?? []).map((item, i) => (
            <div key={i}
              ...
              <div className="aspect-[4/5] bg-[#111] flex flex-col items-center justify-center gap-3">
                ...
                <span className="font-condensed text-[#444] text-xs uppercase tracking-widest">Sin foto a├║n</span>
              </div>
              <div className="p-4">
    ...
    <section id={category.id} className="py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 md:px-12">
        ...
        <div className="mt-10 flex justify-center">
          <a href={`https://wa.me/${wa}?text=${encodeURIComponent(`Hola, quiero ver m├ís opciones de ${category.label}.`)}`}
            ...
            <WAIcon size={14} />
            Ver m├ís {category.label.toLowerCase()} ÔåÆ
          </a>
        </div>
      </div>
    </section>
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between h-16 md:h-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setOpen(!open)} className="md:hidden flex flex-col gap-1.5 p-1" aria-label="Men├║">
        ...
        <div className="md:hidden bg-[#0F0F0F] border-t border-[#2A2A2A] px-4 py-4 flex flex-col gap-3">
          {cats.map((c) => (
            <button key={c.id} onClick={() => go(c.id)}
              ...
              {c.label}
              {c.comingSoon && <span className="ml-2 text-xs text-[#E8151B]">pr├│ximamente</span>}
            </button>
          ))}
          <a href={`https://wa.me/${wa}`} target="_blank" rel="noopener noreferrer"
    ...
    <section className="grunge-overlay relative min-h-screen flex flex-col items-center justify-center text-center overflow-hidden">
      ...
      <div className="relative z-10 px-4 max-w-5xl mx-auto flex flex-col items-center gap-8 pt-24">
        ...
        <p className="font-condensed text-lg md:text-2xl text-[#888] uppercase tracking-[0.25em] max-w-md">
          Ropa urbana aut├®ntica ÔÇö lo que la calle necesita
        </p>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="text-center md:text-right">
          ...
          <p className="font-condensed text-[#444] text-xs uppercase tracking-widest mt-4">
            ┬® {new Date().getFullYear()} URB&amp;STREET ÔÇö Real Style. Everyday.
          </p>
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-[#F5F5F5]">
      ...
      <div className="bg-[#25D366] py-4">
        <a href={waLink(wa)} target="_blank" rel="noopener noreferrer"
          ...
          <WAIcon size={24} />
          ┬┐Listo para pedir? Escr├¡benos ahora por WhatsApp ÔåÆ
        </a>
      </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <p className="font-condensed text-[#444] text-xs mt-1">
              Si no pones imagen, se usar├í un placeholder.
            </p>
          </div>
...
function AdminCategoryView({
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <p className="font-condensed text-[#888] text-sm uppercase tracking-widest">
          {category.products.length} productos ÔÇö {visible} visibles
        </p>
      ...
      {category.products.length === 0 ? (
        <div className="border border-dashed border-[#2A2A2A] py-16 flex flex-col items-center gap-4">
          ...
          <p className="font-condensed text-[#444] text-sm uppercase tracking-widest">
            Sin productos ÔÇö agrega el primero
          </p>
        </div>
      ) : (
...
function AdminSettings({
  ...
  return (
    <div>
      ...
      <p className="font-condensed text-[#555] text-sm uppercase tracking-widest mb-8">
        Configuraci├│n global de la tienda
      </p>
      <div className="max-w-md">
        <form onSubmit={save} className="bg-[#0F0F0F] border border-[#2A2A2A] p-6">
          ...
          <label className="font-condensed text-xs uppercase tracking-widest text-[#888] block mb-1">
            N├║mero (solo d├¡gitos, con c├│digo de pa├¡s)
          </label>
          ...
          <button type="submit"
            ...
            {saved ? "┬íGuardado!" : "Guardar n├║mero"}
          </button>
        </form>
      </div>
    </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-56 bg-[#0A0A0A] border-r border-[#1A1A1A] flex-none hidden md:flex flex-col overflow-y-auto py-6">
          ...
          <div className="px-4 mt-4">
            <p className="font-condensed text-[10px] text-[#333] uppercase tracking-[0.3em] mb-3">Categor├¡as</p>
            {cats.map((c) => {
